...
class AML():
    ...
    def __get_report(self, train_path, dev_path, model_list=[], train=True):
        ...
        for model_name in tqdm(model_list):
            try:
                ...
                model = self.__get_one_model(
                    model_name, df_train, df_dev, train=train)
                # get dev/test report
    ...
    def fit(self, train_path, dev_path, model_list=[]):
        df_report = self.__get_report(
            train_path, dev_path, model_list=model_list, train=True)
        return df_report

    def train(self, train_path, dev_path, model_list=[]):
        return self.fit(train_path, dev_path, model_list=model_list)
    ...
    def evaluate(self, df_path, model_list):
        ...
        for model_name in tqdm(model_list):
            try:
                # get model
                model = self.__get_one_model(
                    model_name, df_train=None, df_dev=None, train=train)
